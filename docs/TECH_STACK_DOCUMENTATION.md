...
- [1] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/_layout.tsx#L2
- [2] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/index.tsx#L1
- [3] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/_layout.tsx#L2
- [4] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(auth)/_layout.tsx#L1
- [5] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(auth)/login.tsx
- [6] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(auth)/sign-up.tsx

## 3. หน้า Home (หน้าแรกของแอป)

หน้า Home ซึ่งนิยามในไฟล์ `app/(tabs)/index.tsx` [7, 8, 9] ทำหน้าที่เป็นจุดเริ่มต้นหลักของการใช้งาน PinnitApp โดยแสดงรายการปักหมุด (pins) ทั้งหมดที่ผู้ใช้สามารถมองเห็นได้ในขณะนั้น ซึ่งอาจมาจากสองแหล่งคือ รายการที่เก็บในเครื่อง (local storage) และรายการที่เก็บในฐานข้อมูล Supabase (กรณีผู้ใช้ล็อกอิน) เมื่อผู้ใช้กดปุ่ม "ปักหมุดตำแหน่งปัจจุบัน" ในหน้า Home ระบบจะอ่านตำแหน่ง GPS ปัจจุบันของผู้ใช้ เรียกใช้บริการ reverse geocoding เพื่อหาชื่อสถานที่ และสร้างรายการปักหมุดใหม่เพิ่มเข้าไปในรายการ
...
ในเชิงเทคนิค หน้า Home ใช้ `useFocusEffect` ในการโหลดรายการปักหมุดทุกครั้งที่หน้าได้รับโฟกัส ทำให้เมื่อผู้ใช้เพิ่ม แก้ไข หรือ ลบปักหมุดจากหน้าจออื่นแล้วกลับมาหน้าแรก ข้อมูลจะถูกอัปเดตอย่างถูกต้องเสมอ การแสดงผลแต่ละรายการแยกออกเป็นคอมโพเนนต์ย่อยชื่อ `PinItem` [10] ซึ่งรองรับการปัดเพื่อลบ การแสดงสถานะ read-only และการตอบสนองต่อการกดค้างเพื่อแก้ไขชื่อ โดยมีไฟล์ `utils/pinsSync.ts` [11] เป็นตัวกลางในการโหลด บันทึก และซิงค์ข้อมูลระหว่าง local storage และ Supabase

- [7] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L93
- [8] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L28
- [9] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L315
- [10] Ref: https://github.com/cherd8524/PinnitApp/blob/master/components/PinItem.tsx
- [11] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/pinsSync.ts

## 4. FlatList

การแสดงรายการปักหมุดในหน้า Home ใช้คอมโพเนนต์ `FlatList` ของ React Native [12, 13, 14] เพื่อรองรับรายการจำนวนมากอย่างมีประสิทธิภาพ `FlatList` จะทำงานด้วยแนวคิด virtualization คือเรนเดอร์เฉพาะรายการที่มองเห็นบนหน้าจอและบริเวณใกล้เคียง ช่วยลดการใช้หน่วยความจำและทำให้การเลื่อนหน้าจอราบรื่น โครงสร้างของ `FlatList` จะกำหนดให้ใช้ฟังก์ชัน `renderItem` ในการนำข้อมูลแต่ละรายการ (pin) ไปแสดงด้วยคอมโพเนนต์ `PinItem` [15, 16] ซึ่งภายในจัดการทั้งชื่อปักหมุด เวลา การตอบสนองต่อการแตะ การกดค้าง และการปัดเพื่อลบ
...
- [12] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L528
- [13] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L328
- [14] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L530
- [15] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L531
- [16] Ref: https://github.com/cherd8524/PinnitApp/blob/master/components/PinItem.tsx

## 5. Fetch (Data Fetching)

ในส่วนของการดึงข้อมูลจากบริการภายนอก PinnitApp ใช้ Web API `fetch` ผ่านฟังก์ชันช่วยเหลือที่นิยามไว้ในไฟล์ `utils/geocoding.ts` [17, 18] ฟังก์ชันหลักคือ `getLocationName(lat, lon)` ซึ่งรับค่าละติจูดและลองจิจูด แล้วใช้ `fetch` เรียก API reverse geocoding เพื่อนำพิกัดดังกล่าวไปแปลงเป็นชื่อสถานที่ที่มนุษย์อ่านเข้าใจได้ การตั้งค่า URL และคีย์สำหรับการเชื่อมต่อบริการนี้ถูกอ่านมาจากไฟล์ `.env` ผ่านตัวแปร เช่น `GEOCODE_API_URL` และ `GEOCODE_API_KEY` ทำให้สามารถเปลี่ยนแปลงปลายทางของ API ได้โดยไม่ต้องแก้ไขโค้ดหลักของแอป
...
- [19] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/storage.ts#L1
- [20] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/storage.ts#L5
- [21] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/storage.ts#L6
- [22] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/pinsSync.ts#L6
- [23] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/pinsSync.ts#L7
- [24] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/pinsSync.ts#L8
- [25] Ref: https://github.com/cherd8524/PinnitApp/blob/master/lib/supabase.ts#L1
- [26] Ref: https://github.com/cherd8524/PinnitApp/blob/master/lib/supabase.ts#L14
...
ในหน้า Home การกดปุ่มเพิ่มปักหมุดจะทำให้ระบบอ่านตำแหน่งปัจจุบันผ่าน `expo-location` แล้วสร้างรายการปักหมุดใหม่ ในขณะที่หน้าแผนที่ (`app/(tabs)/map.tsx`) [31, 32, 33, 34, 35, 36, 37, 38] จะแสดงคอมโพเนนต์ `MapView` แบบเต็มหน้าจอ โดยมี `Marker` สำหรับแต่ละปักหมุดที่มีอยู่ในระบบ รวมถึง marker สำหรับตำแหน่งปัจจุบันของผู้ใช้ด้วย การควบคุมมุมมองของแผนที่จะใช้วัตถุ `Region` เพื่อระบุศูนย์กลางและระดับการซูม และสามารถสั่งให้แผนที่เลื่อนหรือซูมไปยังตำแหน่งของปักหมุดใดปักหมุดหนึ่งเมื่อผู้ใช้เลือกจากหน้า Home

- [27] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L19
- [28] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L115
- [29] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L123
- [30] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/index.tsx#L134
- [31] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L13
- [32] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L14
- [33] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L377
- [34] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L393
- [35] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L411
- [36] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L509
- [37] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L524
- [38] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/map.tsx#L538

## 8. Supabase (Backend & Database)

Supabase ทำหน้าที่เป็น Backend หลักของ PinnitApp ทั้งในส่วนของการยืนยันตัวตน (Authentication) และฐานข้อมูลปักหมุด (Database) แอปใช้ไลบรารี `@supabase/supabase-js` ในการสร้าง client ผ่านไฟล์ `lib/supabase.ts` [39, 40, 41] โดยกำหนดค่า URL และ Anon Key จากตัวแปรในไฟล์ `.env` ด้าน Authentication แอปออกแบบให้ผู้ใช้ล็อกอินด้วย “username” แต่ภายในจะถูกแปลงเป็นอีเมลในรูปแบบ `username@pinnit.local` เพื่อใช้ร่วมกับระบบของ Supabase ได้สะดวก เมื่อผู้ใช้เข้าสู่ระบบสำเร็จ session จะถูกเก็บไว้ใน AsyncStorage ทำให้ผู้ใช้ไม่ต้องล็อกอินใหม่ทุกครั้งที่เปิดแอป
...
ในหน้าตั้งค่า (`app/(tabs)/settings.tsx`) [47, 48] เมื่อผู้ใช้ที่ล็อกอินแล้วแตะที่วงกลมรูปโปรไฟล์ ระบบจะแสดง **action sheet** (เมนูตัวเลือกแบบ bottom sheet) ให้เลือกได้ดังนี้
...
เมื่อผู้ใช้เลือกถ่ายภาพหรือเลือกรูปจากคลัง รูปจะถูกส่งในรูปแบบ base64 ไปยังฟังก์ชัน `uploadProfileImage()` ใน `utils/avatarUpload.ts` [44, 45, 46] ฟังก์ชันนี้จะแปลง base64 เป็น ArrayBuffer ด้วยไลบรารี `base64-arraybuffer` แล้วเรียก `supabase.storage.from('pinnit-app').upload()` พร้อมกำหนด `upsert: true` เพื่อเขียนทับรูปเดิมของผู้ใช้คนเดียวกัน เมื่ออัปโหลดเสร็จ แอปจะอัปเดต `user_metadata` ด้วย URL รูปใหม่และค่า `avatar_updated_at` เพื่อให้ UI แสดงรูปล่าสุด

- [43] Ref: https://github.com/cherd8524/PinnitApp/blob/master/lib/supabase.ts
- [44] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/avatarUpload.ts#L27
- [45] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/avatarUpload.ts#L36
- [46] Ref: https://github.com/cherd8524/PinnitApp/blob/master/utils/avatarUpload.ts#L45
- [47] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/settings.tsx#L44
- [48] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/settings.tsx#L116

## 10. Supabase (Auth)

Supabase Auth ทำหน้าที่เป็นระบบยืนยันตัวตนหลักของ PinnitApp สำหรับการล็อกอิน สมัครสมาชิก และออกจากระบบ แอปใช้รูปแบบอีเมล–รหัสผ่าน โดยเชื่อมต่อผ่าน client ใน `lib/supabase.ts` [49, 50, 51] เดียวกับ Backend และ Storage และกำหนดให้เก็บ session ใน AsyncStorage พร้อมเปิด `persistSession` เพื่อให้ผู้ใช้ไม่ต้องล็อกอินใหม่ทุกครั้งที่เปิดแอป หน้าการสมัครและล็อกอินอยู่ที่ `app/(auth)/sign-up.tsx` [52] และ `app/(auth)/login.tsx` [53] ตามลำดับ
...
ในหน้าตั้งค่า การ์ดบัญชีผู้ใช้ (เมื่อล็อกอินแล้ว) แสดงรูปโปรไฟล์หรือตัวอักษรแรกของชื่อ ชื่อแสดง และชื่อผู้ใช้/อีเมล ทางขวาของการ์ดมีไอคอนแก้ไข (pencil-outline) เมื่อกดจะเปิด โมดัลเปลี่ยนชื่อโปรไฟล์ ให้กรอกชื่อแสดง (full_name) แล้วบันทึกผ่าน `supabase.auth.updateUser({ data: { ...user_metadata, full_name } })` เพื่ออัปเดต metadata โดยไม่เปลี่ยนอีเมลหรือรหัสผ่าน [54]
...
- [49] Ref: https://github.com/cherd8524/PinnitApp/blob/master/lib/supabase.ts#L12
- [50] Ref: https://github.com/cherd8524/PinnitApp/blob/master/lib/supabase.ts#L22
- [51] Ref: https://github.com/cherd8524/PinnitApp/blob/master/lib/supabase.ts#L38
- [52] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(auth)/sign-up.tsx#L112
- [53] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(auth)/login.tsx#L63
- [54] Ref: https://github.com/cherd8524/PinnitApp/blob/master/app/(tabs)/settings.tsx#L88